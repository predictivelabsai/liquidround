...
- **New specialist prefixes** (`scan:`, `triage:`, `intent:`, `buyers:`, `comps:`, `ltm:`, `dcf:`, `multi:`, `synergy:`, `vdr:`, `abstract:`, `legal:`, `ops:`, `esg:`, `memo:`, `teaser:`, `bid:`, `ipo:`, `score:`, `research:`, `integrate:`, `hedgefunds:`, `filings:`, `releases:`, `write-release:`, `ir-triage:`, `ir-compliance:`, `ir-publish:`, `ir-distribute:`) -- `render_agent._specialist()` invokes the LangGraph agent via `cached_agent(slug).ainvoke(...)`, extracts the final `AIMessage`, and renders both the text bubble and any `__ARTIFACT__` payloads as inline tables / citations.
...
**`tools/` layer** -- LangChain StructuredTools consumed by the 32 agents. Each tool is a sync function wrapped with `StructuredTool.from_function(...)` and, where relevant, emits an artifact via `tools.artifact.emit(...)` (which prepends `__ARTIFACT__` + JSON). Current tools:

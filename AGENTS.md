...
repo (opencode, Claude Code, Cursor, etc.). `CLAUDE.md` mirrors this content.

## Mandatory local UI validation

Before committing or pushing any user-facing change, use the repository skill
`.opencode/skills/validate-liquidround-ui/SKILL.md`.
...
**Route wiring** -- all in `main.py`'s top-level block. Order matters: `routes/landing.py` is mounted **first** so `/` resolves to the landing, not the chat. Mounted routers (in order): `landing`, `tools`, `blog`, `auth`, `upload`, `research`, `api`, `pipeline`, `companies`, `digest`, `memo_pdf`, `valuation`, `game`, `analytics`, `exports`, `dataroom`, `ipo_map`, `ipo_pipeline`, `prospectus`, `hedge_funds`, `filings`, `press_releases`, `investor_relations`, `spacs`, `help`, `instructions`, `deal_radar`. `routes/home.py`, `routes/deals.py`, and `routes/market.py` exist on disk but are **not mounted** -- dead code.
...
- **Specialist prefixes** (`scan:`, `triage:`, `intent:`, `buyers:`, `comps:`, `ltm:`, `dcf:`, `multi:`, `synergy:`, `vdr:`, `abstract:`, `legal:`, `ops:`, `esg:`, `memo:`, `teaser:`, `bid:`, `ipo:`, `score:`, `research:`, `integrate:`, `hedgefunds:`, `filings:`, `releases:`, `rto:`, `write-release:`, `ir-triage:`, `ir-compliance:`, `ir-publish:`, `ir-distribute:`) -- `render_agent._specialist()` invokes the LangGraph agent via `cached_agent(slug).ainvoke(...)`, extracts the final `AIMessage`, and renders both the text bubble and any `__ARTIFACT__` payloads as inline tables / citations.
...
- **Memo -> PDF pipeline** (`chat_memo_pdf.py`, mounted as `/app/memo-pdf/*`) -- after an IC Memo / Teaser response, the client POSTs rendered markdown to `/app/memo-pdf/render`, which generates a reportlab PDF and returns a `file_id`. The client opens `/app/memo-pdf/file/<file_id>` in a dedicated browser tab. Content-addressed (sha1) so identical memos reuse cached PDFs.
- **3-pane chat shell** (`components/chat_shell.py`) -- ported from pehero: left pane (Sessions / Agents / Workspace / Configuration), center pane (header + messages + chat input), right pane (always-visible desktop News feed). Composed in `main.py`'s `/app` route. `center_pane()` accepts `readonly=True` for the shared chat view.
...
**`tools/` layer** -- LangChain StructedTools consumed by the agents. Each tool is a sync function wrapped with `StructuredTool.from_function(...)` and, where relevant, emits an artifact via `tools.artifact.emit(...)` (which prepends `__ARTIFACT__` + JSON). Current tools:
...
- **Adding a new agent entry:** append an `AgentSpec` to `agents/registry.py` (bump the `assert len(AGENTS) == N` to match), add `prompts/system/<slug>.md`, and create `agents/<category>/<slug>.py` with `SPEC` + `TOOLS` + `build() -> callable`. Add the prefix to the router's `_best_in_category_for` if it should win on keyword matches. Re-run `pytest tests/test_registry.py`.
...
- **User Guide must stay current.** When adding a new feature, page, or agent, update `docs/user_guide.md` to document it. The guide is rendered at `/app/user-guide` (also `/app/help`) by `routes/help.py` which converts the markdown to styled FastHTML components. Guide images go in `docs/img/`. The TOC is auto-generated from `## ` headings.
- **Commit style:** sentence-case, one short line describing the change (see `git log --oneline`). Commits created by AI agents are co-authored per the project default. Do not commit or push unless explicitly asked.
...
- `agents/base_agent.py` -- the old `BaseAgent` ABC, still used by `scoring_agent.py`, `valuer.py`, `target_finder.py`, `research_agent.py`, `document_agent.py`, `orchestrator.py`. The new 26-agent architecture uses `agents/base.py` (`cached_agent`, `build_simple_agent`) -- different file, different pattern, don't confuse them.

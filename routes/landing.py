...
def landing_platform():
    return landing_page(
        Div(
            Div(
                H1("The platform.",
                   ...
                   style=f"color:{INK};"),
                P("The ECM Agent Squad — 22 specialists across 5 workflow categories (sourcing, underwriting, diligence, capital, portfolio). ECM stands for Equity Capital Markets. Same app for buyers and sellers.",
        ...
        Div(
            Div(
                H2("Built on the stack you'd choose.",
                   cls="text-2xl font-bold tightest text-center mb-10",
                   style=f"color:{INK};"),
                Div(
                    *[Div(
                        Span(name, cls="text-sm font-medium mono", style=f"color:{INK};"),
                        P(blurb, cls="text-xs mt-1", style=f"color:{INK_MUTED};"),
                        cls="card rounded-lg p-4",
                    ) for name, blurb in [
                        ("FastHTML + HTMX", "Server-rendered hypermedia. No SPA bloat."),
                        ("LangChain + LangGraph", "ECM squad prompts, one router, swap providers freely."),
                        ("xAI Grok (swappable)", "Fast, cheap. Swap to OpenAI or Anthropic via factory."),
                        ("yfinance + EXA + Tavily", "Real company data + semantic search + fresh news."),
                        ("PostgreSQL", "Workflows, deals, documents, pipeline — all durable."),
                        ("Plotly", "Live charts embedded directly in chat answers."),
                    ]],
    ...
    return landing_page(
        Div(
            ...
            H1("The ECM Agent Squad.",
               ...
               style=f"color:{INK};"),
            P(
                "22 specialist agents for Equity Capital Markets — M&A and IPO, buyer and seller. Click any card for details and example prompts.",
...
def landing_pricing():
    tiers = [
        ("Starter", "Free", "For solo advisors exploring LiquidRound.",
         ["Full ECM Agent Squad (22 agents)", "yfinance + public research", "Up to 20 queries / day", "Community support"], False),
        ("Team", "From $499 / month", "For boutique M&A and PE teams.",
         ["Everything in Starter", "Unlimited queries", "Deal & document workspace", "Priority support", "Team pipelines"], True),
